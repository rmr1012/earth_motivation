...
})(window,document,'script','notrack','ga');
 ...
 * @type {string}
 */
const ANALYTICS_ID = 'UA-54021204-2';
...
function trackClick(event) {
  ...
  if (type === 'bookmarks-link') {
    ...
    ga('send', 'event', {
      eventCategory: 'type',
      eventAction: element.getAttribute('title')
    });
  } else {
    ga('send', 'event', {
      eventCategory: 'type',
...
const webglDetected = detectWebGL();
const browser = getBrowser();
const linkToEarth = browser === 'chrome' && webglDetected;

window.earthview.config = {
  // This is the template to build the maps link.
  MAPS_LINK: 'https://www.google.com/maps/@{{lat}},{{lng}},{{zoom}}z/' +
    'data=!3m1!1e3',
  // This is the template to build the earth link.
  EARTH_LINK: 'https://earth.google.com/web/' +
    '@{{lat}},{{lng}},{{elevation}}a,{{range}}d,30y,270h,30t,0r/data=KAI',
  // The base url to the images.
  IMAGE_BASE_URL: 'https://www.gstatic.com/prettyearth/assets/data/v2/',
  // The deeplink base url to the earthview gallery.
  BASE_URL: 'https://earthview.withgoogle.com/',
  SHARE_BASE_URL: 'https://g.co/ev/',
  WEBGL_DETECTED: webglDetected,
  LINK_TO_EARTH: linkToEarth,
  /**
   * Flag to hide or show teasers.
   * @type {boolean}
   */
  DISABLE_TEASER: true
};
...
/**
 * Returns if the current browser has WebGl support or not.
 * @return {boolean} WebGl support.
 */
...
/**
 * Get the browser of the extension.
...
backgrounds.Globe = new Model({
  ...
  init(options) {
    ...
    this.data = backgrounds.GlobeData;
    this.current = [0, 0];
    this.$pointer = document
      .querySelector('.content__location__globe__pointer');
    this.projection = d3.geoOrthographic()
      .scale(this.options.size / 2)
      .translate([this.options.size / 2, this.options.size / 2])
      .clipAngle(90)
      .rotate([-this.current[0], -this.current[1]]);
    ...
    this.world = this.svg.append('path');

    this.world
      .datum(
        topojson.feature(
          this.data,
          this.data.objects.land
        )
      )
      .attr('class', 'land');
  ...
  set(photo) {
    ...
    const distance = this.distance(from[1], from[0], to[1], to[0]);
    const duration = Math.max(distance * 1500, 500);
    ...
    d3.transition()
      ...
      .tween('rotation', () => function(t) {
        ...
        this.projection.rotate([-lng3, -lat3]);
...
backgrounds.History = new Model({
   ...
   */
  isInHistory(id) {
    const index = this.history.map(element => element.id).indexOf(id);
   ...
   */
  render() {
    const template = $('#history-item');
    const placeholder = $('#placeholder-item');
    ...
    for (let i = this.history.length; i < 10; i++) {
      const historyItem = this.createHistoryItem(null, placeholder);
      this.$history.appendChild(historyItem);
    }
  },
  ...
  createHistoryItem(item, template) {
    ...
    const country = item.geocode.country;
    const location = item.geocode.locality ||
...
backgrounds.App = new Model({
  ...
  /**
   * Initialies the teaser.
   */
  initTeaser() {
    if (earthview.config.DISABLE_TEASER) {
      return;
    }

    const $teasers = document.querySelectorAll('.teaser');
    const today = new Date();
    const daysSince = Math.floor(today / (24 * 60 * 60 * 1000));
    const $currentTeaser = $teasers[daysSince % $teasers.length];

    this.teaser = new backgrounds.Teaser($currentTeaser);
  },
   ...
   */
  showChanges() {
    const $container = document.querySelector('.overlay--changes');
...
backgrounds.Menu = new Model({
  ...
  init(history) {
    ...
    const mapsLinkText = this.$container
      .querySelector('.menu__item--maps span');
    mapsLinkText.textContent = earthview.config.LINK_TO_EARTH
...
backgrounds.Photo = new Model({
  ...
  show(id) {
    ...
    if (id) {
      ...
      earthview.app.history.history.forEach(photo => {
        if (photo.id === id) {
   ...
   */
  saveImage(newImage) {
    const oldImage = this.storage.getImage();
  ...
  /**
   * Returns the URL to Google Earth.
   * @returns {string} The URL to Earth.
   */
  getEarthLink() {
    const radius = 6378137;
    const factor = 40;
    const zoom = this.data.zoom + 1 || 11;
    const scale = Math.pow(2, zoom);
    const radiant = this.data.lat * (Math.PI / 180);
    const distance = Math.abs(Math.cos(radiant)) / scale * radius;
    const range = Math.round(distance * factor);
    const elevation = Math.round(this.data.elevation || 0);

    return earthview.config.EARTH_LINK
  ...
  /**
   * Returns the URL to Google Maps.
   * @returns {string} The URL to Maps.
   */
  getMapsLink() {
    const {zoom = 10, lat, lng} = this.data;

    return earthview.config.MAPS_LINK
      .replace('{{lat}}', lat)
      .replace('{{lng}}', lng)
      .replace('{{zoom}}', zoom + 1);
  },
  ...
  /**
   * Returns the URL to either Google Maps or Google Earth.
   * @returns {string} The URL.
   */
  getLink() {
    const link = earthview.config.LINK_TO_EARTH
      ? this.getEarthLink()
      : this.getMapsLink();
  ...
  /**
   * Returns the text for the link to either Google Maps or Google Earth.
   * @returns {string} The text.
   */
  getLinkText() {
    const {country, location} = this.getCountryAndLocation();
    const inProductName = earthview.config.LINK_TO_EARTH
      ? 'in Google Earth'
      : 'on Google Maps';

    return `See ${location}, ${country} ${inProductName}`;
  },

  /**
   * Return the country and the location of the photo
   * @returns {object} With country and location as keys.
   */
  getCountryAndLocation() {
    const country = this.data.geocode.country;
    const location = this.data.geocode.locality ||
  ...
  /**
   * Returns the attribution for a photo.
   * @returns {string} The attribution.
   */
  getAttribution() {
    const copyright = `©${(new Date()).getFullYear()}`;
    const attribution = this.data.attribution || '';

    // replace the original with the current year
    return attribution.replace(/©\s?\d{4}/, copyright);
  },
  ...
  render() {
    ...
    const downloadLink
      = `${earthview.config.BASE_URL}download/${this.data.id}.jpg`;
    const {country, location} = this.getCountryAndLocation();
    const linkTitle = this.getLinkText();

    this.$background.style.backgroundImage = `url(${this.data.dataUri})`;
    this.$downloadLink.setAttribute('href', downloadLink);
    this.$attributionText.textContent = this.getAttribution();
   ...
   */
  getImageUrl(data) {
    const base64 = data.dataUri.replace('data:image\/jpeg;base64,', '');
    const blob = new Blob([this.base64DecToArr(base64)], {type: 'image/jpeg'});
...
backgrounds.Share = new Model({
  ...
  shareDeeplink(event) {
    ...
    this.photo.getThumbnail(440, 250).then(image => {
  ...
  share(shareUrl) {
    window.open(
      shareUrl.replace('#{shareUrl}', earthview.config.SHARE_BASE_URL) +
        this.photo.getId(),
      'Share',
...
backgrounds.Storage = new Model({
   ...
   */
  hasImage() {
    return Boolean(localStorage.getItem('image'));
  },
...
backgrounds.Teaser = new Model({
  /**
   * Initializes a single teaser.
   * @param {Element} $teaser The teaser container
   */
  init($teaser) {
    this.$teaser = $teaser;
    this.$dismiss = this.$teaser.querySelector('.teaser__dismiss');
    this.$goTo = this.$teaser.querySelectorAll('.teaser__go-to');

    const dismissedTeaser = localStorage.getItem('dismissedTeaser');

    if (JSON.parse(dismissedTeaser) !== true) {
  ...
  /**
   * Remove the teaser from the page.
   */
  remove() {
    localStorage.setItem('dismissedTeaser', true);
